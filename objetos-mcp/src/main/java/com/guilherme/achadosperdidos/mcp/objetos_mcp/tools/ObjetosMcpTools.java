...
public class ObjetosMcpTools {
    ...
    @Tool(description = "Busca objetos por nome, categoria, cor, marca ou descrição para verificar se um item semelhante já existe e evitar duplicidade antes de cadastrar um novo objeto.")
    public List<ObjetoDTO> buscarObjetosPorFiltro(
            ...
            @ToolParam(description = "Cor principal do objeto (ex: preta, azul)") String cor,
            @ToolParam(description = "Marca do objeto (se aplicável)") String marca,
            @ToolParam(description = "Descrição detalhada do objeto") String descricao) {

        return objetosClient.buscarPorAtributos(nome, categoria, cor, marca, descricao);
    }

    @Tool(description = """
            Cadastra um novo objeto no catálogo do sistema.
            Antes de criar, verifica se um cadastro parecido já existe; se existir, reutiliza o objeto cadastrado para evitar duplicidade.
    ...
    public ObjetoDTO cadastrarObjeto(
        ...
        List<ObjetoDTO> objetosExistentes = buscarObjetosPorFiltro(nome, categoria, cor, marca, descricao);
        if (!objetosExistentes.isEmpty()) {
            return objetosExistentes.stream()
                    .filter(objeto -> normalizar(objeto.getNome()).equals(normalizar(nome))
                            && normalizar(objeto.getCor()).equals(normalizar(cor))
                            && (objeto.getCategoria() == null || normalizar(objeto.getCategoria()).equals(normalizar(categoria))))
                    .findFirst()
                    .orElse(objetosExistentes.getFirst());
        }

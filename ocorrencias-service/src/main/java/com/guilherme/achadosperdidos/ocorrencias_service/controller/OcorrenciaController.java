...
public class OcorrenciaController {
    ...
    @GetMapping("/ocorrencias/objeto/{id}")
    public ResponseEntity<OcorrenciaDTO> buscarOcorrenciaPorObjetoId(@PathVariable Long id) {
        return service.getOcorrenciaByObjetoId(id)
                .map(ResponseEntity::ok)
                .orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PostMapping("/ocorrencias")
    public ResponseEntity<OcorrenciaDTO> registrarOcorrencia(@RequestBody OcorrenciaDTO ocorrencia) {
        log.info("[OCORRENCIA-CONTROLLER] Registrando Ocorrencia: {}", ocorrencia);
        return ResponseEntity
                .status(HttpStatus.CREATED)
                .body(service.insert(ocorrencia));
    }

...
public class OcorrenciasMcpTools {
    ...
    @Tool(description = "Busca os detalhes de uma ocorrencia especifica Pelo o ID do Objeto.")
    public OcorrenciaDTO buscarOcorrenciaPorObjetoId(@ToolParam(description = "ID do Objeto") Long id) {
        return ocorrenciasRestClient.buscarPorObjetoId(id);
    }

    @Tool(description = """
            Registra uma nova ocorrência de objeto perdido ou achado no campus do IFBA.
            Retorna a ocorrência persistida, incluindo o campo id. Esse id é o ocorrenciaId
            que deve ser usado posteriormente para solicitar uma reivindicação; não confunda
            esse id com o objetoId recebido como parâmetro.
